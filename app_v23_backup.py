...
MENUS = [
    {
        ...
        "story_type": "🌿 음식문화 이야기",
        "story": "등 푸른 생선을 소금에 간해 구워 먹는 방식은 오래된 일상 식문화에 가깝습니다. 특별한 조리도구가 없어도 불과 소금만 있으면 한 끼가 완성되었고, 그래서 생선구이는 바쁜 날에도 든든하게 먹는 집밥의 상징처럼 자리 잡았습니다."
    },
    {
        "name": "소고기 버섯전골",
        "category": "한식",
        ...
        "story_type": "🌿 음식문화 이야기",
        "story": "전골은 여러 재료를 한 냄비에 모아 함께 익혀 먹는 음식입니다. 한 사람의 몫을 따로 차리기보다 한 상에 둘러앉아 같은 냄비를 나누는 방식이라, 가족이나 손님과 함께 먹는 자리와 잘 어울리는 음식으로 여겨졌습니다."
    },
    {
        "name": "닭고기 채소덮밥",
        "category": "한식",
        ...
        "story_type": "🌿 음식문화 이야기",
        "story": "밥 위에 반찬을 얹어 한 그릇으로 먹는 방식은 시간과 공간을 아끼는 실용적인 식사 문화와 잘 맞습니다. 바쁜 날에도 밥과 단백질, 채소를 한 번에 챙길 수 있어 현대적인 한 그릇 식사의 대표적인 형태가 되었습니다."
    },
    {
        "name": "두부 버섯 비빔밥",
        "category": "한식",
        ...
        "story_type": "🌿 음식문화 이야기",
        "story": "비빔밥은 서로 다른 반찬을 한 그릇 안에서 섞어 새로운 맛을 만드는 음식입니다. 남은 나물과 밥을 알뜰하게 활용하는 생활의 지혜와도 연결되며, 여러 재료가 한데 어우러진다는 점에서 조화의 음식으로 자주 이야기됩니다."
    },
    {
        "name": "에그 치킨 샌드위치 세트",
        "category": "양식",
        ...
        "story_type": "📜 역사 일화",
        "story": "샌드위치라는 이름은 18세기 영국의 샌드위치 백작과 관련된 유명한 일화에서 널리 알려졌습니다. 식사 도중 손을 많이 쓰지 않고도 먹을 수 있도록 빵 사이에 고기를 넣어 먹었다는 이야기가 전해지며, 오늘날에는 빠르고 간편한 한 끼의 상징이 되었습니다."
    },
    {
        "name": "연어 샐러드와 고구마",
        "category": "양식",
        ...
        "story_type": "🌿 음식문화 이야기",
        "story": "연어는 북쪽 바다와 강을 오가는 생선으로 오래전부터 여러 지역에서 귀한 단백질원으로 여겨졌습니다. 오늘날에는 신선한 채소와 함께 가볍게 먹는 방식이 널리 퍼지면서, 부담은 줄이고 만족감은 챙기는 식사의 이미지가 강해졌습니다."
    },
]
...
def render_story(menu, style, context):
    base = menu.get("story", "")
    story_type = menu.get("story_type", "🌿 음식문화 이야기")
    special = context.get("special", "").strip()
    if style == "짧고 실용적으로":
        return story_type, base.split(".")[0].strip() + "."
    if style == "따뜻한 감성 이야기":
        tail = " 오늘 한 끼가 조금 더 편안하고 기억에 남는 시간이 되면 좋겠습니다."
        if special:
            tail = f" 오늘의 일인 ‘{special}’와도 잘 어울리는, 잠시 쉬어가는 한 끼가 되면 좋겠습니다."
        return story_type, base + tail
    if style == "우화처럼":
        return "✨ 오늘을 위한 창작 이야기", "옛날 어느 작은 마을의 식탁에 이런 음식이 올랐다고 상상해볼까요? " + base + " 서로 다른 재료가 제자리를 지키면서도 한 끼 안에서 어우러진다는 점이 오늘의 작은 교훈처럼 느껴집니다."
    return story_type, base
        ...
        with st.form("quick_recommend_form"):
            ...
            with st.expander("＋ 상세 조건 추가"):
                ...
                with e2:
                    ...
                    story_style = st.radio(
                        "추천 설명 스타일",
                        ["재미있는 역사 이야기", "짧고 실용적으로", "따뜻한 감성 이야기", "우화처럼"],
                        index=0
                    )
        ...
        if st.session_state.recommendations:
            ...
            for i, menu in enumerate(st.session_state.recommendations, 1):
                ...
                with st.container(border=True):
                    ...
                    story_style = st.session_state.last_context.get("story_style", "재미있는 역사 이야기")
                    story_type, story_text = render_story(menu, story_style, st.session_state.last_context)
                    with st.expander("📖 오늘의 음식 이야기", expanded=True):
                        st.caption(story_type)
                        st.write(story_text)
                        if story_type == "📜 역사 일화":
                            st.caption("※ 널리 알려진 역사 일화를 짧게 소개한 내용입니다.")
                        elif story_type == "✨ 오늘을 위한 창작 이야기":
                            st.caption("※ 실제 역사 기록이 아닌, 오늘의 식사를 위한 창작 우화입니다.")
...
st.divider()
st.caption("MVP v0.3 · 생활계량은 이해를 돕기 위한 대략적인 기준이며, 실제 식품 크기와 조리 상태에 따라 달라질 수 있습니다.")
